...
| `budget_consumed` | Measured `polls` and `rows`. A live run adds `execution_ms` (submit to terminal status, downloads excluded) and the bounds it ran under: `poll_quota` and `execution_timeout_ms` (the client-side execution deadline, `--statement-timeout` + 5 s per statement). With `MAX_CREDITS`: `microcredits_per_hour`, `max_microcredits` and `estimated_microcredits` (millionths of a credit; advisory). |
